...
static const uint8_t KEY_MIXED[32] = {
    0x76, 0x10, 0xEA, 0xC8, 0xC4, 0x50, 0x73, 0xC6,
    0x8D, 0xC5, 0x04, 0x51, 0x08, 0x86, 0x60, 0x5E,
    0x49, 0xA5, 0x38, 0xDE, 0x3D, 0xA4, 0xBA, 0xEF,
    0xF3, 0xE2, 0x29, 0x98, 0x8B, 0xAD, 0xA9, 0x49,
};

// 期望签名 SHA-256 指纹段（密文）：PART[i] = realHash[i] ^ xorKey[i]
static const uint64_t PART[4] = {
    0x6DB444C90ACB9792ULL,
    0xFA3437CE605CE0A9ULL,
    0xA78BBA3BDA6EF219ULL,
    0xA8393334CC8D0C7CULL,
};

#if 0
// 旧存储方式（结构体相邻存放，已弃用，保留历史对照；不参与编译）
constexpr ObfuscatedHash EXPECTED = {                                                                                   
    { // part (混淆后的哈希段)                                                                                          
        0x6DB444C90ACB9792ULL,  // part[0]                                                                              
        0xFA3437CE605CE0A9ULL,  // part[1]                                                                              
        0xA78BBA3BDA6EF219ULL,  // part[2]                                                                              
        0xA8393334CC8D0C7CULL,  // part[3]                                                                              
    }, { // xorKey (解密密钥)                                                                                           
        0x68DEFC6F6243B8D1ULL,  // xorKey[0]                                                                            
        0xE8D532BBE3B57522ULL,  // xorKey[1]                                                                            
        0x4117089674910DEEULL,  // xorKey[2]                                                                            
        0xFF1C19382A98525CULL,  // xorKey[3]                                                                            
    }                                                                                                                   
};  
